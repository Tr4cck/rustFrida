...
mod tests {
    ...
    #[test]
    fn test_gpr_state_new() {
        let state = GPRState::new();
        assert_eq!(state.x0, 0);
        assert_eq!(state.pc, 0);
    }

    #[test]
    fn test_gpr_state_get_set() {
        let mut state = GPRState::new();
        state.set(0, 0x12345678);
        assert_eq!(state.get(0), Some(0x12345678));
        assert_eq!(state.x0, 0x12345678);
    }

    #[test]
    fn test_fpr_state_new() {
        let state = FPRState::new();
    ...
    #[test]
    fn test_vm_options() {
        let opts = VMOptions::DISABLE_FPR | VMOptions::BYPASS_PAUTH;
        assert!(opts.contains(VMOptions::DISABLE_FPR));
        assert!(opts.contains(VMOptions::BYPASS_PAUTH));
        assert!(!opts.contains(VMOptions::ENABLE_BTI));
    }

    #[test]
    fn test_version() {
        let (version, version_str) = get_version();
        assert!(version > 0 || version_str == QBDI_VERSION_STRING);
    }
}
